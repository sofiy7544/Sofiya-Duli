...
const CHANNEL_LABEL: Record<Channel, string> = {
  TELEGRAM: 'Telegram', WHATSAPP: 'WhatsApp', EMAIL: tr('Почта'), INSTAGRAM: 'Instagram',
};
...
export function InboxScreen() {
  ...
  if (!enabled) {
    return (
      <PageBody>
        <PageHeader title={tr('Коммуникации')} />
        <EmptyState icon={MessagesSquare} title={tr('Мессенджеры не подключены')}
          text={tr('Когда администратор подключит Telegram, WhatsApp или почту, переписка появится здесь.')}
          action={<Link href="/settings" className="inline-flex h-11 items-center justify-center gap-2 rounded-control border border-border bg-surface px-3.5 text-sm font-medium transition-colors hover:bg-surface-2 lg:h-9">{tr('Открыть настройки')}</Link>} />
      </PageBody>
    );
  }

  const header = (
    <PageHeader title={tr('Коммуникации')} subtitle={tr('Обращения из всех каналов в одном списке')} />
  );
  if (r.error) return <PageBody>{header}<ErrorState error={r.error} onRetry={r.retry} what={tr('переписку')} /></PageBody>;
  ...
  if (items.length === 0) {
    return <PageBody>{header}<EmptyState icon={MessagesSquare} title={tr('Новых обращений нет')} text={tr('Сообщения из Telegram, WhatsApp, почты и Instagram будут приходить сюда.')} /></PageBody>;
  }
          ...
          <li key={c.id}>
            <button type="button" onClick={() => setOpenId(c.id)}
              ...
              <div className="mt-2 flex flex-wrap items-center gap-1.5">
                ...
                {c.status === 'new' && <StatusBadge tone="info">{tr('Новое')}</StatusBadge>}
                {c.status === 'qualified' && <StatusBadge tone="success">{tr('Лид создан')}</StatusBadge>}
                {c.status === 'dismissed' && <StatusBadge>{tr('Не лид')}</StatusBadge>}
                {c.duplicateOf && <StatusBadge tone="warning">{tr('Возможный дубликат')}</StatusBadge>}
              </div>
            </button>
          </li>
        );
      ...
      return (
        <PageBody>
          <div className="mb-3 flex items-center gap-1">
            ...
            <span className="t-caption">{tr('Коммуникации')}</span>
          </div>
          <Thread c={current} />
        </PageBody>
      );
  ...
  return (
    <PageBody>
      ...
      <div className="grid grid-cols-[minmax(280px,360px)_1fr] items-start gap-4">
        ...
        {current ? <Thread c={current} /> : <div className="surface grid min-h-[320px] place-items-center p-6"><p className="t-caption">{tr('Выберите обращение слева.')}</p></div>}
      </div>
    </PageBody>
  );
}
    ...
    <section className="surface overflow-hidden">
        ...
        <div className="flex items-start gap-2.5 border-b border-border bg-warning/10 px-4 py-3">
          ...
          <div className="text-[13.5px]">
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              ...
              <Button size="sm" variant="outline" onClick={() => toast.success(tr('Обращение привязано к карточке клиента'))}>{tr('Это тот же человек')}</Button>
            </div>
          </div>
        </div>
      )}
      ...
      {c.status === 'new' ? (
        <div className="border-t border-border bg-surface-2/50 px-4 py-3">
          <p className="text-[13.5px] font-medium">{tr('Это обращение — лид?')}</p>
          <p className="t-caption mt-0.5">{tr('Создадим карточку лида и перенесём переписку в его историю.')}</p>
        ...
        <div className="border-t border-border px-4 py-2.5">
          <p className="t-caption">{c.status === 'qualified' ? tr('Лид по этому обращению уже создан.') : tr('Помечено как «не лид».')}</p>
        </div>
      )}
