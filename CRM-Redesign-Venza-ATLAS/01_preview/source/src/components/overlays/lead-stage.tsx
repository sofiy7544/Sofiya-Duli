...
const LOST_REASONS = [tr('Купил через другое агентство'), tr('Не устроила цена'), tr('Отложил покупку'), tr('Не выходит на связь')];
...
export function LostReasonSheet({ open, onOpenChange, onConfirm }: { open: boolean; onOpenChange: (o: boolean) => void; onConfirm: (reason: string) => Promise<void> }) {
  ...
  const submit = async () => {
    if (!final) { setError(tr('Выберите причину — без неё лид нельзя закрыть')); return; }
    setBusy(true); try { await onConfirm(final); onOpenChange(false); } finally { setBusy(false); }
  };
  return (
    <Sheet open={open} onOpenChange={onOpenChange} title={tr('Почему лид проигран?')} description={tr('Причина попадёт в отчёт по потерям.')} desktop="center" size="sm"
      footer={<><Button variant="outline" className="flex-1" onClick={() => onOpenChange(false)}>{tr('Отмена')}</Button><Button variant="destructive" className="flex-1" loading={busy} onClick={submit}>{tr('Закрыть лид')}</Button></>}>
      <div role="radiogroup" aria-label={tr('Причина')} className="space-y-2">
        {[...LOST_REASONS, 'other'].map((r) => (
          <button key={r} role="radio" aria-checked={reason === r} onClick={() => { setReason(r); setError(null); }}
            ...
            {r === 'other' ? tr('Другая причина') : r}
          </button>
        ))}
        {reason === 'other' && <Field label={tr('Опишите причину')} required>{(id) => <Textarea id={id} maxLength={500} value={custom} onChange={(e) => setCustom(e.target.value)} autoFocus />}</Field>}
...
export function StageSheet({ lead, open, onOpenChange, onPick }: { lead: Lead | null; open: boolean; onOpenChange: (o: boolean) => void; onPick: (s: LeadStage) => void }) {
  return (
    <Sheet open={open} onOpenChange={onOpenChange} title={tr('Этап лида')} desktop="center" size="sm">
          ...
          return (
            <li key={s}>
              <button disabled={!allowed && !current} onClick={() => { onOpenChange(false); if (!current) onPick(s); }}
                ...
                {current && <Check className="h-4 w-4 text-primary" aria-label={tr('Текущий')} />}
                {!allowed && !current && s === 'WON' && <span className="t-caption">{tr('после «Переговоров»')}</span>}
              </button>
            </li>
          );
        })}
