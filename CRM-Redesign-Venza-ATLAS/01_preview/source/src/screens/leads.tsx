...
export function LeadsScreen() {
  ...
  const body = () => {
    if (r.error) return <ErrorState error={r.error} onRetry={r.retry} what="воронку" />;
  ...
  return (
    <PageBody wide={isDesktop && view === 'board'}>
      ...
      <div className="mb-4 flex items-center gap-2">
        ...
        {activeFilterCount(filters) > 0 && <Button variant="ghost" size="sm" onClick={() => setFilters({})}>Сбросить</Button>}
        <div className="flex-1" />
        {isDesktop && isAdmin && view === 'board' && <Button size="sm" variant={selectMode ? 'soft' : 'ghost'} onClick={() => { setSelectMode(!selectMode); setSelected([]); }}>{selectMode ? 'Готово' : 'Выбрать'}</Button>}
      ...
      {selectMode && selected.length > 0 && (
        <div role="region" aria-label="Массовые действия" className="material fixed bottom-6 left-1/2 z-50 flex -translate-x-1/2 items-center gap-2 rounded-[18px] border border-[var(--glass-border)] p-2 pl-4 shadow-lift animate-pop-in">
          <span className="whitespace-nowrap text-[14px] font-semibold tabular">Выбрано: {selected.length}</span>
    ...
    <Sheet open={!!lead} onOpenChange={(o) => !o && onClose()} title={client?.fullName ?? ''} description={lead ? `Этап: ${STAGE_LABEL[lead.stage]}` : undefined} desktop="center" size="sm">
      {lead && (<div className="space-y-3">
        <div className="surface-quiet p-1.5">
          ...
          <button className={row} onClick={() => onStage(lead)}><Workflow className="h-5 w-5 text-primary" aria-hidden />Сменить этап</button>
          <button className={row} onClick={() => { onClose(); router.navigate(`/leads/${lead.id}`); }}><LayoutList className="h-5 w-5 text-primary" aria-hidden />Открыть карточку</button>
        </div>
        <div className="surface-quiet p-1.5"><button className={cn(row, 'text-danger-text')} onClick={() => onLost(lead)}><XCircle className="h-5 w-5" aria-hidden />Закрыть как проигранный</button></div>
      </div>)}
    </Sheet>
  );
}
...
function Board({ leads, onMove, family, selectMode, selected, onToggle, onStage }: { leads: Lead[]; onMove: (l: Lead, s: LeadStage) => Promise<boolean>; family: string; selectMode: boolean; selected: string[]; onToggle: (id: string) => void; onStage: (l: Lead) => void }) {
  ...
  const drop = async (s: LeadStage) => {
    ...
    if (!isStageTransitionAllowed(l.stage, s)) { toast.error('Этот переход недоступен'); return; }
            ...
            <div className="flex min-h-[140px] flex-1 flex-col gap-2 px-2 pb-2">
              {s === 'LOST' ? (
                ...
                <div key={l.id} draggable={!selectMode} onDragStart={(e) => { setDragId(l.id); e.dataTransfer.effectAllowed = 'move'; e.dataTransfer.setData('text/plain', l.id); }} onDragEnd={() => { setDragId(null); setOver(null); }}
                  ...
                  {selectMode && <button onClick={() => onToggle(l.id)} role="checkbox" aria-checked={selected.includes(l.id)} aria-label="Выбрать лид" className="absolute inset-0 z-10 flex items-start justify-end rounded-card p-2">
...
function BoardSkeleton() {
  return <div className="flex gap-3 overflow-hidden" role="status" aria-label="Загрузка">{[0, 1, 2, 3, 4].map((i) => <div key={i} className="w-[268px] shrink-0 space-y-2 rounded-card bg-surface-2/60 p-2"><Skeleton className="m-1 h-4 w-24" />{Array.from({ length: 3 - (i % 2) }).map((_, k) => <Skeleton key={k} className="h-[104px] rounded-card" />)}</div>)}</div>;
}
